...
## Define rutas
DATA_PATH = "/opt/sisepuede/ref/ingestion/calibrated/india"
SSP_SECTOR_DATA_PATH = {ssp_sector : os.path.join(DATA_PATH, f"model_input_variables_india_{ssp_sector}_calibrated.xlsx") for ssp_sector in ssp_sectores}
...
FILE_PATH = "/opt/sisepuede/docs/source/csvs/attribute_dim_time_period.csv"

time_period_dim = pd.read_csv(FILE_PATH)

time_period_dim_extendido = pd.DataFrame([ [(year_extend_original - year_initial) + i + 1, year_extend_original + i + 1]  for i in range(year_extend - year_extend_original)], columns = ["time_period", "year"])

time_period_dim = pd.concat([time_period_dim, time_period_dim_extendido], ignore_index = True)

time_period_dim.to_csv(FILE_PATH, index = False)