...
for idx, country in enumerate(countries):
    ...
    for sector in sisepuede_sector:
        ...
        carpetas_sector = [i for i in glob.glob(f"/opt/sisepuede_data/{sector}/*") if len(i.split("_")) >2 ]

        #print(f"{len(historical_subsector_files)}-{len(projected_subsector_files)}")
        print(sector)
        if sector!= "Energy":
            ...
            for ssp_var in carpetas_sector:
                try:
                    variable = ssp_var.split("/")[-1]
                    historical_file = os.path.join(ssp_var, "input_to_sisepuede", "historical", f"{variable}.csv")
                    projected_file = os.path.join(ssp_var, "input_to_sisepuede", "projected", f"{variable}.csv")
